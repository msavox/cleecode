...
integrated terminals, syntax highlighting, and a Turbo Pascal/Borland-style menu bar —
all inside your terminal.

![clicode main view](docs/screenshots/main.png)
...
- **Menu bar** — a macOS-style `CliCode` app menu (About / Settings / Quit) plus File / Edit / View / Terminal, all keyboard- and mouse-driven, with shortcuts shown inline
...
- **Drag & drop** — drop a file onto the file tree to copy it in; drop it onto a terminal running an active `ssh` session and clicode attempts an `scp` upload
...
```bash
cargo build --release
./target/release/clicode            # opens the current directory
./target/release/clicode src/main.rs  # opens the current directory with a file pre-opened
```
...
Personal project, actively evolving. Not affiliated with any other "clicode"-named project.
