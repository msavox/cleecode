...
| `python_ws_test.py` | The Python side, end to end. **Pins `TERM="dumb"`,** which makes CPython fall back to the basic REPL — see the correction in `docs/ide-mode-python.md`, since that pin is why the prompt hook was measured as firing once per statement. Anything checking the hook's *cadence* has to run without it. |
