...
impl Index {
    ...
    pub fn add_buffer(&mut self, rope: &Rope, near_line: Option<usize>) {
        ...
        let (from, to) = match near_line {
            Some(cursor) => {
                (cursor.saturating_sub(SCAN_WINDOW), cursor.saturating_add(SCAN_WINDOW).min(lines))
            }
...
mod tests {
    ...
    fn a_servers_first_suggestion_outranks_a_word_from_across_the_file() {
        ...
        let near =
            vec![cand("config_path", Source::Buffer, 0, 1), cand("config_x", Source::Lsp, 4, 1)];
    ...
    fn keywords_sort_last_even_on_an_exact_prefix() {
        let cands = vec![
            cand("impl", Source::Keyword, 0, 1),
            cand("implementation", Source::Buffer, 300, 1),
        ];
    ...
    fn frequency_breaks_a_tie_at_equal_distance() {
        let cands =
            vec![cand("value_one", Source::Buffer, 5, 1), cand("value_two", Source::Buffer, 5, 7)];
